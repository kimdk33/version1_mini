...
while cap.isOpened():
    ...
    for box in results[0].boxes:
        ...
        try:
            detect_stopped_car, stopped_id = detect_highway_stopped_vehicle(
                tid, cx, cy
            ...
            if detect_stopped_car:
                ...
                file_name = (
                    f"parking[{tid}]_{stopped_car_datetime.strftime("%Y_%m_%d_%H_%M_%S")}"
                )
        ...
        if df.loc[tid, "speed"] is not None:
            ...
            detect_wrong_way_car = wrong_way_drive(cls, cx, cy, car_direction, speed_px1)


            # 차선에 따른 속력보정후 실제 속력
            real_speed = get_real_speed(cls, cx)
            ...
            if detect_wrong_way_car:
                ...
                # file_name 예시 parking_25-12-19_13:00:00.mp4
                file_name = (
                    f"wrong_way[{tid}]_{wrong_way_datetime.strftime("%Y_%m_%d_%H_%M_%S")}"
                )
                recording_start[tid] = [(frame_num + 90) % 180, file_name, cx, cy]
                # column = ["type", "direction", "speed", "datetime", "illegal", "file_name"]
                df.loc[tid, ["illegal", "file_name"]] = ["wrong_way", f'{file_name}.mp4']
